# USAGE
# python motion_cv_save.py --output contoh.avi
...
for frame in camera.capture_continuous(rawCapture, format="bgr", use_video_port=True):
	...
	if writer is None:
		...
		(h, w) = frame.shape[:2]
		writer = cv2.VideoWriter("{}/{}.avi".format(args["output"], dt.datetime.now().strftime("PICAM_%d%m%Y_%H%M%S")), fourcc, args["fps"],
			(w , h ), True)
		zeros = numpy.zeros((h, w), dtype="uint8")
	
	output = numpy.zeros((h , w , 3), dtype="uint8")
	output = frame
